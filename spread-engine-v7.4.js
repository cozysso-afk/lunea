...
/* ===== V7.4 EVENT-STAGE + SPECIALTY ADDON ===== */
'use strict';
/* LUNEA Spread Engine V7.4 main addon
...
(() => {
  ...
  if(!BASE){console.error('[LUNEA V7.4] V7 base engine missing');return;}
  ...
  const TIME_LABELS = {
    today:'오늘/당일', nextday:'내일/모레', week:'이번 주', month:'이번 달', m1_3:'1~3개월', m3_6:'3~6개월',
  ...
  function detectTime(q){
    ...
    if(/내일|모레/i.test(q))return'nextday';
  ...
  function detectEventStage(q){
    // 같은 명사(소개팅/면접/계약 등)라도 사건 단계를 먼저 구분한다.
    // 우선순위: 아직 기회가 생기기 전 > 명시적 사후 경험 > 예정된 사건 전 > 일반/진행.
    const intro=/(소개팅|첫\s*만남|첫\s*데이트)/i.test(q);
    const introOffer=/(제의|제안|주선|소개\s*받|소개해\s*(?:준|주|줄)|잡힐|잡히|생길|생기|들어올|들어오|기회|연결될|연결되|성사될)/i.test(q);
    if((intro&&introOffer)||/(소개해\s*(?:준|주|줄)).{0,24}소개팅/i.test(q))return'pre_opportunity';
    if(/(?:면접|채용).{0,24}(제안|제의|연락\s*(?:올|오)|잡힐|잡히|기회|오퍼|들어올)|(?:제안|제의|오퍼).{0,18}(?:면접|채용)/i.test(q))return'pre_opportunity';
    if(/(?:계약|협찬|광고|외주|프로젝트|제휴|일감).{0,24}(제안|제의|문의|오퍼|들어올|들어오|연락\s*(?:올|오))|(?:제안|제의|문의|오퍼).{0,18}(?:계약|협찬|외주|프로젝트|제휴)/i.test(q))return'pre_opportunity';
    // "오늘 면접 보고 왔는데"처럼 시간어가 있어도 이미 겪은 사건이면 사후 평가가 우선.
    if(/(?:소개팅|첫\s*만남|첫\s*데이트).{0,18}(후|직후|끝나고|하고\s*나서|만나고|만난\s*뒤|봤는데|다녀와|애프터|연락\s*텀|첫인상|감상)|(?:애프터|첫인상|연락\s*텀).{0,18}(소개팅|첫\s*데이트|상대)/i.test(q))return'post_event';
    if(/(?:면접|미팅|상견례|계약).{0,22}(후|직후|끝나고|보고\s*(?:나서|왔|옴)|봤는데|다녀와|결과|평가|반응)|(?:오늘|어제).{0,12}(?:면접|미팅|상견례).{0,12}(?:보고|봤|다녀)/i.test(q))return'post_event';
    if(/(?:소개팅|첫\s*만남|첫\s*데이트|면접|미팅|상견례|계약).{0,18}(예정|앞두|가기\s*전|보기\s*전|만나기\s*전|하기\s*전)|(?:오늘|내일|모레|이번\s*주).{0,14}(소개팅|첫\s*만남|첫\s*데이트|면접|미팅|상견례)/i.test(q))return'pre_scheduled';
    return'ongoing';
  }

  function detectSpecialty(q){
    // 매우 구체적인 질문은 넓은 카테고리보다 먼저 잡는다.
    const eventStage=detectEventStage(q);
    // 사건명보다 사건 단계/동사를 우선한다.
    if(eventStage==='pre_opportunity' && /소개팅|첫\s*만남|첫\s*데이트|소개해\s*(?:준|주|줄)/i.test(q)) return 'blind_date_offer';
    if(eventStage==='pre_opportunity' && /면접|채용/i.test(q)) return 'interview_offer';
    if(eventStage==='pre_opportunity' && /계약|협찬|광고|외주|프로젝트|제휴|일감/i.test(q)) return 'deal_offer';
    if(eventStage==='pre_scheduled' && /소개팅|첫\s*만남|첫\s*데이트/i.test(q)) return 'first_date_upcoming';
    ...
    const R=[
      ...
      ['first_date',/(?:소개팅|첫\s*만남|첫\s*데이트).{0,18}(?:후|직후|끝나고|하고\s*나서|만나고|만난\s*뒤|애프터|첫인상|감상|연락\s*텀)|애프터/i],
      ...
      ['money_flow',/(?:이번\s*달|이번달|월간|월별).{0,18}(?:금전|자금|지출|수입|재정)|(?:금전|자금|재정).{0,18}(?:이번\s*달|월간|흐름)|예상치\s*못한\s*지출/i],
  ...
  function enrich(base){
    ...
    return {...base,question:q,specialty,structure,timeRange,targetScope,eventStage,domains,risk:uniq(risk),symbolicMode,
      axes:{structure:LABELS[structure]||structure,time:TIME_LABELS[timeRange]||timeRange,target:TARGET_LABELS[targetScope]||targetScope,stage:{pre_opportunity:'기회 발생 전',pre_scheduled:'예정 사건 전',post_event:'사건 후 평가',ongoing:'진행/일반'}[eventStage]||eventStage}};
  }
  ...
  function specialtyCandidates(s){
    ...
    switch(id){
      ...
      case'blind_date_offer':
        C.push(makeSp('소개팅 제의 · 유입/시기 5','아직 상대를 만나기 전 질문이므로 첫인상·애프터를 다루지 않고, 소개 기회의 유입 여부와 경로·지연요인·기간 내 전조를 본다','v74-blind-date-offer',[
          '질문한 기간 안에 소개팅 제안이나 주선이 들어오는 흐름을 지지하는 현재 신호',
          '소개 기회가 생기는 것을 늦추거나 막는 현실적·관계적 요인',
          '제안이 들어온다면 가장 가능성이 높은 경로·중개자·상황',
          '질문한 기간 안에서 기회가 살아나는 상대적 구간과 그 전에 보일 전조',
          '기간 내 실제 제의로 이어지는 쪽이 우세한지와 기회를 놓치지 않기 위해 확인할 행동 신호'
        ])); break;
      case'interview_offer':
        C.push(makeSp('면접 제안/기회 · 유입 5','아직 면접을 본 뒤가 아니므로 면접관 평가를 만들지 않고, 지원 흐름·연락 경로·지연요인·기간 내 기회를 본다','v74-interview-offer',[
          '질문한 기간 안에 면접 연락이나 제안이 들어오는 흐름을 지지하는 현재 신호',
          '서류·경력·채용 일정 중 기회를 늦추는 가장 현실적인 요인',
          '기회가 생긴다면 어떤 지원 경로·회사 유형·연결 방식에서 들어오기 쉬운지',
          '기간 안에서 연락 가능성이 살아나는 상대적 구간과 그 전에 보일 전조',
          '실제 면접 기회로 이어지기 위해 내가 확인·보완할 가장 중요한 현실 신호'
        ])); break;
      case'deal_offer':
        C.push(makeSp('계약/협업 제안 · 유입 5','아직 제안이 확정되기 전 단계이므로 성사 후 결과가 아니라 문의·제안 유입, 조건, 지연요인, 현실 신호를 본다','v74-deal-offer',[
          '질문한 기간 안에 제안·문의가 유입되는 흐름을 지지하는 현재 신호',
          '제안이 늦어지거나 끊길 수 있는 현실적 병목',
          '제안이 들어온다면 가장 가능성이 높은 경로·상대 유형·프로젝트 성격',
          '제안이 실제 협상 단계로 넘어가기 전에 확인해야 할 조건·예산·범위',
          '기간 내 실제 기회로 연결되는 쪽이 우세한지 가르는 현실 행동 신호'
        ])); break;
      case'first_date_upcoming':
        C.push(makeSp('예정된 소개팅 · 첫 만남 준비 5','이미 약속이 잡힌 만남 전 질문이므로 결과를 미리 확정하지 않고, 컨디션·첫인상 형성·대화 포인트·리스크·후속 가능성을 준비 관점에서 본다','v74-first-date-pre',[
          '만남 당일 내가 가장 자연스럽게 살릴 수 있는 분위기와 강점',
          '상대에게 첫인상으로 전달되기 쉬운 나의 핵심 이미지',
          '대화가 잘 풀리게 만드는 공통 관심사·소통 포인트',
          '과한 긴장·기대·말실수 등 첫 만남의 흐름을 막을 수 있는 변수',
          '만남 후 다음 연락이나 애프터로 이어지기 위해 필요한 상호 신호'
        ])); break;
      case'first_date':
        C.push(makeSp('소개팅·첫 데이트 직후 · 인상/애프터형','이미 만남을 겪은 뒤의 질문일 때만 첫인상·호감·아쉬움·후속 행동을 분리','v74-first-date-post',['만남 직후 상대가 받은 전체적인 첫인상','이성적 호감으로 연결된 가장 강한 포인트','아쉬움·거리감 또는 망설임을 만든 포인트','애프터/다음 연락을 실제로 하게 만드는 조건','후속 연락 텀이 길어진다면 가장 가능성 높은 이유'])); break;
      case'social_opinion':
      ...
      case'money_flow':
        C.push(makeSp('월간 금전 흐름 · 수입/지출/변수 5','돈이 들어온다/나간다를 한 장으로 뭉개지 않고 고정 흐름·예상 밖 변수·완충 여력을 분리','v74-money-flow',[
          '이번 기간의 기본적인 현금 유입·유출 균형',
          '예상치 못한 지출이나 자금 압박을 만들 가능성이 큰 변수',
          '반대로 숨통을 틔울 수입·절약·회수 기회',
          '내 소비·투자·상환 판단에서 과하게 낙관하거나 놓치기 쉬운 지점',
          '기간 말 재정 안정도를 높이기 위해 먼저 지킬 현실 기준'
        ])); break;
      case'creator_growth':
  ...
  function axesText(s){return `구조 ${s.axes.structure} · 시간 ${s.axes.time} · 단계 ${s.axes.stage} · 대상 ${s.axes.target}${s.specialty?` · 특화 ${s.specialty}`:''}`;}
  ...
  async function designSpreadV72(question){
    ...
    result.designRationale=`${result.designRationale} · V7.4 메타: ${axesText(sem)} · 로컬 후보 ${c.length}개 중 최근 의미중복을 피해서 선택`;
    remember(sem,result);
    console.info('[LUNEA V7.4]',sem,result);
    return result;
  }
  ...
  function directiveV72(){
    ...
    if(!sem)return base;
    const extra=[];
    extra.push(`\n[V7.4 질문 메타]\n- 질문 구조: ${sem.axes.structure}\n- 시간 범위: ${sem.axes.time}\n- 사건 단계: ${sem.axes.stage}\n- 관계/대상 범위: ${sem.axes.target}\n- 같은 주제어보다 질문의 동사와 사건 단계가 우선이다. 아직 일어나지 않은 사건에는 사건 후 평가(첫인상/애프터/면접평가 등)를 끌어오지 않는다. 이미 겪은 사건에는 아직 기회가 생길지 묻는 유입 템플릿을 쓰지 않는다.`);
  ...
  W.LUNEA_SPREAD_ENGINE_V74 = {analyze:(q)=>enrich(BASE.analyze(q)),specialtyCandidates,structuralFallback,designSpread:designSpreadV72,memory,detectEventStage};

  const badge=document.querySelector('.engine-strip span:last-child');
  if(badge)badge.innerHTML='<b>Secure Draw + Spread V7.4</b> · Intent + 사건단계 우선 · 초세부 주제 라우팅 · 안전형 사실/반증 분리';
  const aiItem=[...document.querySelectorAll('.reading-item')].find(x=>x.dataset?.title==='질문 맞춤 AI 배열');
  if(aiItem){
    aiItem.dataset.desc='V7.4가 질문 구조·시간 범위·사건 단계·관계 대상을 먼저 태깅하고, 같은 주제어라도 사건 전/후를 구분해 초세부 템플릿으로 자동 배치합니다.';
    const p=aiItem.querySelector('p');if(p)p.textContent='4축 태깅(구조·시간·사건단계·대상) → 초세부 주제 매핑 → 후보 비교 → 범위이탈 차단.';
    const cnt=aiItem.querySelector('.count');if(cnt)cnt.textContent='AI 2~12';
  }
  console.info('🌙 LUNEA Spread Engine V7.4 main addon loaded');
})();
