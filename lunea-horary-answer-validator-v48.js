...
(() => {
  ...
  function strongPositive(text) {
    ...
    const positive = /(성사(?:된다|될\s*가능성이\s*(?:높|크)|가능성이\s*(?:높|크))|재회(?:한다|할\s*수\s*있|가능성이\s*(?:높|크))|관계\s*회복(?:이\s*된다|가능성이\s*(?:높|크))|연락(?:이\s*온다|이\s*올\s*가능성이\s*(?:높|크)|할\s*가능성이\s*(?:높|크))|답장(?:이\s*온다|이\s*올\s*가능성이\s*(?:높|크))|합격(?:한다|할\s*가능성이\s*(?:높|크))|채용(?:된다|될\s*가능성이\s*(?:높|크))|계약(?:이\s*성사된다|성사\s*가능성이\s*(?:높|크)))/i;
