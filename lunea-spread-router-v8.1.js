...
/*
  LUNEA SPREAD ROUTER V8.1 — STRUCTURE FIRST + SAFE BOOT
  ------------------------------------------------
  목적
  ...
  lunea-spread-router-v8.1.js  <-- 가장 뒤

  기본 호환 카드 상한: 12
  렌더러가 22장까지 확실히 지원할 때만:
    window.LUNEA_MAX_CUSTOM_CARDS = 22;
  를 이 파일보다 먼저 설정하면 11 x 2 완전 대칭 배열을 허용한다.
*/
(() => {
  const W = window;
  ...
  if (W.__LUNEA_SPREAD_ROUTER_V81_BOOTSTRAP__) return;
  W.__LUNEA_SPREAD_ROUTER_V81_BOOTSTRAP__ = true;
  ...
  function maxCards() {
    const configured = Number(W.LUNEA_MAX_CUSTOM_CARDS);
    if (Number.isFinite(configured) && configured >= 6) {
  ...
  function analyzeV8(question) {
    ...
    try {
      ...
      console.warn('[LUNEA Router V8] previous analyze failed:', e);
    }
  ...
  async function designSpreadV8(question) {
    ...
    saveTrace(trace);
    console.info('[LUNEA Router V8]', trace, result);
    return result;
  }
...
- 포지션이 담당하지 않는 다른 축으로 의미를 확장하지 않는다.`;
  ...
  W.readingDirective = function(...args) {
    ...
    try {
      ...
      console.warn('[LUNEA Router V8] previous readingDirective failed:', e);
    }
    ...
    selfTest: () => {
      const samples = [
        ...
        },
        {
          ...
          expectNot: 'comparison'
        }
      ];

      return samples.map(t => {
        const a = analyzeV8(t.q);
        const ok = t.expect ? a.primary_intent === t.expect : a.primary_intent !== t.expectNot;
        return {
          name: t.name,
          ok,
          primary_intent: a.primary_intent,
          target_count: a.target_count,
          target_count_mode: a.target_count_mode,
          target_relation: a.target_relation,
          axes: a.requested_axis_ids
        };
      });
    }
  };
  ...
  const updateUi = () => {
    try {
      ...
      if (badge) {
        badge.innerHTML = `<b>Secure Draw + Spread Router V8</b> · 구조/대상수 우선 · 대칭 비교 · 요구축 커버리지`;
      }
      const aiItem = [...document.querySelectorAll('.reading-item')]
        .find(x => x.dataset?.title === '질문 맞춤 AI 배열');
      if (aiItem) {
        aiItem.dataset.desc = 'V8이 대상 수·비교 구조·사용자 요구축을 먼저 읽고, 기존 배열이 질문을 못 덮으면 동적 대칭/직결 배열로 전환합니다.';
        const p = aiItem.querySelector('p');
        if (p) p.textContent = '대상 수 → 비교 구조 → 요구축 → 주제 → 기존 배열 적합도 → 필요 시 동적 배열.';
        const cnt = aiItem.querySelector('.count');
        if (cnt) cnt.textContent = `AI 2~${maxCards()}`;
      }
    } catch (e) {
      console.warn('[LUNEA Router V8] UI update skipped:', e);
    }
  };
  ...
  if (document.readyState === 'loading') {
    ...
    console.info('🌙 LUNEA Spread Router V8.1 loaded safely', {
      maxCards: maxCards(),
  ...
  function bootWhenReady() {
    if (initRouterV81()) return;
    if (Date.now() - BOOT_STARTED_AT >= BOOT_TIMEOUT_MS) {
      console.error(
        '[LUNEA Router V8.1] V7.4 engine was not ready within 15s. ' +
