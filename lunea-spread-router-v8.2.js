...
/*
  LUNEA SPREAD ROUTER V8.2 — DEEP 22 + SAFE PERFORMANCE
  ------------------------------------------------
  목적
  ...
  lunea-spread-router-v8.2.js  <-- 가장 뒤
...
(() => {
  ...
  if (W.__LUNEA_SPREAD_ROUTER_V82_BOOTSTRAP__) return;
  W.__LUNEA_SPREAD_ROUTER_V82_BOOTSTRAP__ = true;
  ...
  function analyzeV8(question) {
    ...
    try {
      ...
      console.warn('[LUNEA Router V8.2] previous analyze failed:', e);
    }
  ...
  // ------------------------------------------------------------
  // DEEP 22 PERFORMANCE LAYER
  // - 카드 난수/추첨 결과는 절대 건드리지 않는다.
  // - 13장 이상 심층 배열에서만 입장 애니메이션/이미지 디코딩 부하를 줄인다.
  // - 개별 카드 탭 flip은 유지한다.
  // ------------------------------------------------------------
  let deepDomObserver = null;
  let deepOptimizePending = false;

  function installDeepPerformanceStyles() {
    if (document.getElementById('luneaV82DeepPerfStyle')) return;
    const style = document.createElement('style');
    style.id = 'luneaV82DeepPerfStyle';
    style.textContent = `
      ...
      @supports (content-visibility:auto) {
        body.lunea-deep-spread .lunea-v82-deep-card {
          content-visibility: auto;
          contain-intrinsic-size: auto 300px;
        }
      }

      body.lunea-deep-spread .lunea-v82-deep-card img {
        -webkit-user-drag: none;
      }

      /* 22장을 한꺼번에 GPU 레이어로 승격하지 않는다. 개별 flip만 기존 CSS 사용. */
      body.lunea-deep-spread .lunea-v82-deep-card:not(.flipped):not(.revealed) {
  ...
  function directCardItems(container) {
    const kids = [...container.children];
    if (kids.length >= 13) return kids;

    const selectors = ['.slot', '.tarot', '.tarot-card', '.card-slot', '[data-card-index]'];
    for (const sel of selectors) {
      const found = [...container.querySelectorAll(sel)];
      if (found.length >= 13) {
        // .tarot itself may be nested inside .slot; prefer the nearest repeated outer item.
        if (sel === '.tarot' || sel === '.tarot-card') {
          const outer = found.map(el => el.closest('.slot,.card-slot') || el);
          return [...new Set(outer)];
        }
        return found;
      }
    }
  ...
  function optimizeDeepCardDOM() {
    deepOptimizePending = false;
    if (!W.LUNEA_DEEP_SPREAD_ACTIVE) return;

    let best = [];
    for (const container of likelyCardContainers()) {
      const items = directCardItems(container);
      if (items.length > best.length) best = items;
    }
    if (best.length < 13) return;

    best.forEach((item, index) => {
      item.classList.add('lunea-v82-deep-card');
      item.querySelectorAll('img').forEach(img => {
        try {
          img.decoding = 'async';
          if (index >= 6) img.loading = 'lazy';
          if ('fetchPriority' in img && index >= 6) img.fetchPriority = 'low';
        } catch {}
      });
    });
  }

  function scheduleDeepOptimize() {
    if (deepOptimizePending || !W.LUNEA_DEEP_SPREAD_ACTIVE) return;
    deepOptimizePending = true;
    const run = () => optimizeDeepCardDOM();
  ...
  function ensureDeepDomObserver() {
    if (deepDomObserver || !document.body) return;
    deepDomObserver = new MutationObserver(() => {
      if (W.LUNEA_DEEP_SPREAD_ACTIVE) scheduleDeepOptimize();
    });
    deepDomObserver.observe(document.body, {childList:true, subtree:true});
  }
  ...
  function updateDeepPerformanceMode(result) {
    ...
    if (deep) {
      ensureDeepDomObserver();
      scheduleDeepOptimize();
    }
  }
  ...
  async function designSpreadV8(question) {
    ...
    saveTrace(trace);
    console.info('[LUNEA Router V8.2]', trace, result);
    return result;
  }
...
- 포지션이 담당하지 않는 다른 축으로 의미를 확장하지 않는다.`;
  ...
  W.readingDirective = function(...args) {
    ...
    try {
      ...
      console.warn('[LUNEA Router V8.2] previous readingDirective failed:', e);
    }
  ...
  W.LUNEA_ROUTER_V8 = {
    ...
    performanceStatus: () => ({
      ...
      mode: W.LUNEA_DEEP_SPREAD_ACTIVE ? 'deep_22_light_render' : 'standard'
    }),
    selfTest: () => {
...
- 최종 결론은 카드상 더 유리한 선택과 그 선택이 유리해지는 조건을 함께 말한다.`;
  ...
  const updateUi = () => {
    try {
      ...
      if (badge) {
        badge.innerHTML = `<b>Secure Draw + Spread Router V8.2</b> · 구조/대상수 우선 · 대칭 비교 · 심층 최대 22장`;
      }
      const aiItem = [...document.querySelectorAll('.reading-item')]
        .find(x => x.dataset?.title === '질문 맞춤 AI 배열');
      if (aiItem) {
        aiItem.dataset.desc = 'V8.2가 대상 수·비교 구조·요구축을 먼저 읽고, 복합 A/B 비교는 필요한 축만큼 최대 22장까지 대칭 확장합니다.';
      ...
      console.warn('[LUNEA Router V8.2] UI update skipped:', e);
    }
  };
  ...
  if (document.readyState === 'loading') {
    ...
    console.info('🌙 LUNEA Spread Router V8.2 deep-22 loaded safely', {
  ...
  function bootWhenReady() {
    if (initRouterV82()) return;
    if (Date.now() - BOOT_STARTED_AT >= BOOT_TIMEOUT_MS) {
      console.error(
        '[LUNEA Router V8.2] V7.4 engine was not ready within 15s. ' +
