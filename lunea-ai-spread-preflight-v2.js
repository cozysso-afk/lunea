...
(() => {
  ...
  function caseContext(question){
    try{
      ...
      const usageText=learning?.formatUsageForPrompt?.(question,2,{category:activeCategory()})||'반복 채택 사용 사례 없음';
      return {text,matches,learnedText,learnedMatches,usageText,stats:{families:book?.familyCount||0,utterances:book?.utteranceCount||0,learned:learned.length,totalLearned:typeof learning?.count==='function'?learning.count():0}};
    }catch(e){console.warn('[LUNEA Preflight V2] casebook lookup failed',e)}
    return {usageText:'반복 채택 사용 사례 없음',text:'관련 사례 없음',learnedText:'사용자 학습 사례 없음',matches:[],learnedMatches:[],stats:{families:0,utterances:0,learned:0,totalLearned:0}};
  }
  function schema(){
    return {
      type:'OBJECT',
  ...
  function openPreview(question,initial){
    ...
    return new Promise(resolve=>{
      ...
      $('luneaSpreadPreviewConfirm').onclick=()=>{
        ...
        finish({...current,spreadTitle:title,positions:numbered,designRationale:String(current?.designRationale||'사례집 기반 AI 배열')+' · PRE-DRAW USER CONFIRMED',_luneaPendingCorrection:pending,_luneaPendingUsage:changed?null:{question,category:activeCategory(),originalSpread:baseline,correctedSpread:{spreadTitle:title,positions:numbered},meta:current?._luneaPreflight||{}}});
      };
  ...
  async function commitCorrectionAfterStart(confirmed){
    ...
    const learning=W.LUNEA_SPREAD_LEARNING_V1;
    try{
      if(pending)learning?.record?.(pending);
      else if(confirmed?._luneaPendingUsage)learning?.recordUsage?.(confirmed._luneaPendingUsage);
    }
  ...
  function install(){
    ...
    drawBtn.onclick=async function(event){
      ...
      try{
        ...
        if(result!==false&&result?.ok!==false)await commitCorrectionAfterStart(confirmed);
