...
test('timed-out Gemini work is actually aborted instead of continuing invisibly', async () => {
  let observedSignal = null;
  const baseFetch = (_input, init = {}) => {
  ...
  const source = read('lunea-astro-stability-v2.js')
    .replace('const GEMINI_TIMEOUT_MS = 75000;', 'const GEMINI_TIMEOUT_MS = 20;');
  vm.runInContext(source, context);

  await assert.rejects(
    window.fetch('https://generativelanguage.googleapis.com/v1beta/models/test:generateContent', {method:'POST'}),
    /AI 요청이 75초를 넘어 중단했어/
  );
