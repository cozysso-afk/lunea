...
test('sector art is zoom-cropped and late icons stay synchronized',()=>{assert.match(burgundy,/transform:scale\(1\.20\)/);assert.match(burgundy,/function observeUi\(\)/);assert.match(burgundy,/MutationObserver/);assert.match(burgundy,/const RELEASE = '40\.5'/)});
...
test('Oracle panel uses V47 mobile artwork presentation',()=>{assert.match(oracle,/backgroundSize='cover'/);assert.match(oracle,/brightness\(1\.14\)/);assert.match(oracle,/min-height:26%/);assert.match(oracle,/oracle_back_v2\.png/)});
