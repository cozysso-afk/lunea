...
test('Oracle panel lifts black levels without changing art assets',()=>{assert.match(oracle,/backgroundSize='contain'/);assert.match(oracle,/filter:none/);assert.match(oracle,/rgba\(31,8,18,\.64\)/);assert.match(oracle,/oracle_back_intimacy_final\.png/)});
