...
test('V33 exposes five presets, accepts 90 inclusive days, rejects 91, and explicitly syncs mirrors', () => {
  ...
  assert.match(source, /\.thai-v33-quick\{display:grid;grid-template-columns:repeat\(5,minmax\(0,1fr\)\);gap:6px;width:100%;box-sizing:border-box;/);
  assert.doesNotMatch(source, /\.thai-v33-chip\[data-days="[^"]+"\]\{[^}]*grid-(?:row|column)/);
  ...
  assert.match(index, /lunea-structural-routing-v4\.js\?v=20260912-thai-quick-owner-1/);
