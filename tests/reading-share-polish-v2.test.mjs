...
const polish = fs.readFileSync(new URL('../lunea-reading-share-polish-v3.js', import.meta.url), 'utf8');
...
assert.match(polish, /dataset\.luneaShareRenderer='v3'/, 'the existing single share button must be rebound to V3');
assert.match(touch, /lunea-reading-share-polish-v3\.js/, 'runtime must load the current polished renderer');
assert.match(touch, /order:9999!important/, 'share button must remain the final action');

console.log('reading-share-polish-v3 contract OK');